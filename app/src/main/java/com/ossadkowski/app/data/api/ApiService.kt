...
interface ApiService {
    ...
    // ── Tasks (dashboard legacy) ──
    @POST("api/tasks/list")
    suspend fun getTasks(@Body request: PaginatedRequest): PaginatedResponse<TaskItem>
    ...
    @GET("api/limity-kredytowe/users/search")
    ...
    suspend fun registerDeviceToken(@Body body: com.ossadkowski.app.fcm.DeviceTokenRequest): Response<Unit>

    @HTTP(method = "DELETE", path = "api/device-tokens", hasBody = true)
    suspend fun unregisterDeviceToken(@Body body: com.ossadkowski.app.fcm.DeviceTokenRequest): Response<Unit>
}
