...
import com.ossadkowski.crm.mobile.databinding.ActivityTransportCenyDetailBinding
import com.ossadkowski.crm.mobile.ui.serwis.theme.CrmTheme

class TransportCenyDetailActivity : BaseDrawerActivity() {

    private lateinit var binding: ActivityTransportCenyDetailBinding
    private val viewModel: TransportCenyDetailViewModel by viewModels()

    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        binding = ActivityTransportCenyDetailBinding.inflate(layoutInflater)
        setContentView(binding.root)

        initSession()

        val requestId = intent.getIntExtra("TRANSPORT_ID", 0)
        if (requestId == 0) {
            finish()
            return
        }
