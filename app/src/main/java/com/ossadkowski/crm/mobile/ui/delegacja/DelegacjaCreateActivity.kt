...
import com.ossadkowski.crm.mobile.databinding.ActivityDelegacjaCreateBinding
import com.ossadkowski.crm.mobile.ui.serwis.theme.CrmTheme

class DelegacjaCreateActivity : BaseDrawerActivity() {

    private lateinit var binding: ActivityDelegacjaCreateBinding
    private val viewModel: DelegacjaCreateViewModel by viewModels()

    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        
        binding = ActivityDelegacjaCreateBinding.inflate(layoutInflater)
        ...
        // Load approved wnioski for the current user in step 1
        val currentUserId = sessionManager.userId
        viewModel.loadApprovedWnioski(currentUserId)

        binding.composeView.apply {
            setViewCompositionStrategy(ViewCompositionStrategy.DisposeOnViewTreeLifecycleDestroyed)
            setContent {
                CrmTheme {
                    DelegacjaCreateScreen(
                        viewModel = viewModel,
