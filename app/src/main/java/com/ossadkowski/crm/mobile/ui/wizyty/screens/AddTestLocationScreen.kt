 ...
 * picks a suggestion, and saves it as a geofenced test point. Saving (re)starts the work
 * session so the location is watched immediately — arriving there auto-detects a visit and
...
fun AddTestLocationScreen(
    ...
    Scaffold(
        ...
        Column(
            ...
            Text(
                text = "Wpisz adres testowy. Gdy znajdziesz się w tym miejscu, aplikacja " +
                    "wykryje wizytę i wyśle powiadomienie.",
            ...
            state.selected?.let {
                Text(
                    text = "Lokalizacja: ${"%.5f".format(it.lat)}, ${"%.5f".format(it.lng)}",
