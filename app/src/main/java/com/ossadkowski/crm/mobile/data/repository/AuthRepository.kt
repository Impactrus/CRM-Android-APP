...
class AuthRepository(
    ...
    suspend fun login(username: String, password: String): NetworkResult<LoginResponse> =
        withContext(Dispatchers.IO) {
            // Device info is sent so the backend can register/overwrite this
            // device on every successful login. The client never blocks on
            // device-trust state — that gate (if any) lives server-side.
