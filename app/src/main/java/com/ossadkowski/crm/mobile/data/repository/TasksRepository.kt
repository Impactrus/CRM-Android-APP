...
import com.ossadkowski.crm.mobile.data.cache.CommentPayload
import com.ossadkowski.crm.mobile.data.cache.TaskStatusPayload
...
class TasksRepository(
    ...
    suspend fun getTasksForMonth(rok: Int, miesiac: Int): NetworkResult<List<TaskListItemDto>> {
        // Fetch a large page to approximate getting all tasks for the month
        // In a real scenario, should have a dedicated endpoint
        val result = safeApiCall { apiService.getTasksV2(1, 1000) }
        return when (result) {
            is NetworkResult.Success -> {
    ...
    suspend fun createTask(request: CreateTaskRequest): NetworkResult<Any> {
        val result = safeApiCall { apiService.createTask(request) }
        if (result is NetworkResult.Success) {
            db.invalidateByPrefix("tasks_")
            db.invalidateByPrefix("dash_tasks_")
        } else if (result is NetworkResult.Error) {
            actionQueue.enqueue("create_task", gson.toJson(request))
            return NetworkResult.Success("queued_offline")
        }
        return result
    }
